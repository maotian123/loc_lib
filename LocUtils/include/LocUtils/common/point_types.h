...
inline std::vector<Vec3f> CloudToVec3fBuff(const PointCloudType& cloud)
{
    std::vector<Vec3f> ret;
    ret.resize(cloud.points.size());
    for (size_t i {0}; i < cloud.points.size(); ++i) 
    {
        ret[i] = ToVec3f(cloud.points[i]);
    }
...
POINT_CLOUD_REGISTER_POINT_STRUCT(LocUtils::FullPointType,
                                  (float, x, x)(float, y, y)(float, z, z)(float, range, range)(float, radius, radius)(
                                      ...
                                      std::uint8_t, ring, ring)(double, time_span, time_span)(double, time_intervel, time_intervel)(float, height, height))
...
/// velodyne雷达定义
using VelodynePointCloudType = pcl::PointCloud<Point>;
...
/// Ouster雷达定义
using OusterPointCloudType = pcl::PointCloud<Point>;
...
/// Rslidar雷达定义
using RslidarPointCloudType = pcl::PointCloud<Point>;
