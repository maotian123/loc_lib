#ifndef LocUtils_MATH_UTILS_H
#define LocUtils_MATH_UTILS_H
...
inline float PointDis(const Vec3f& p1, const Vec3f& p2) { return std::sqrt((p1 - p2).squaredNorm()); }

inline float PointDis(const Vec2f& p1, const Vec2f& p2) { return std::sqrt((p1 - p2).squaredNorm()); }
/**
 ...
 * @param {float &} x
 * @param {float} &y
 * @return {float} 返回角度 范围0~2PI
 */
inline float xy2theta(const float & x, const float &y)
{
    if ( x >= 0 && y >= 0) 
        return kRAD2DEG * atan(y / x);

    if ( x < 0 && y >= 0) 
        return 180 - ( kRAD2DEG * atan(y / (-x)) );

    if ( x < 0 && y < 0) 
        return 180 + ( kRAD2DEG * atan(y / x) );

    if ( x >= 0 && y < 0)
        return 360 - ( kRAD2DEG * atan((-y) / x) );
}
