 ...
 * @version: 
 * @Author: lxy
 * @Date: 2023-07-06 17:57:51
 * @LastEditors: lxy
 * @LastEditTime: 2023-07-06 18:11:55
 */
#include "LocUtils/mapping_2d/loop_closing.hpp"
...
void LoopClosing::DebugImageShow(Scan2d::Ptr scan, const SE2& pose, cv::Mat& image, const Vec3b& color, int image_size,
                     float resolution)
{
...
void LoopClosing::MatchInHistorySubmaps() {
    ...
    for (const size_t& can : current_candidates_) {
        ...
        if (mr->AlignG2O(pose_in_target_submap)) {
            ...
            DebugImageShow(current_frame_->scan_, pose_in_target_submap, occu_image, Vec3b(0, 0, 255), 1000, 20.0);
