#include "LocUtils/model/likelihoodField/likelihood_filed.hpp"
#include "LocUtils/common/g2o_types.h"
...
void LikelihoodField::DebugImageShow(Scan2d::Ptr scan, const SE2& pose, cv::Mat& image, const Vec3b& color, int image_size,
                     float resolution)
{
    ...
    // 同时画出pose自身所在位置
    Vec2d pose_in_image =
         (pose.translation()) * double(resolution) + Vec2d(image_size / 2, image_size / 2);
    cv::circle(image, cv::Point2f(pose_in_image[0], pose_in_image[1]), 5, cv::Scalar(color[0], color[1], color[2]), 2);

}
void LikelihoodField::DebugLikeHood(Scan2d::Ptr curscan,Scan2d::Ptr prescan, const SE2& pose)
{
    
    cv::Mat image;
    DebugImageShow(prescan, SE2(), image, Vec3b(255, 0, 0));    // target是蓝的
    DebugImageShow(curscan, pose, image, Vec3b(0, 0, 255));  // source是红的
    cv::imshow("scan", image);
    /// 画出target和它的场函数
    cv::Mat field_image = this->GetFieldImage();
    DebugImageShow(prescan, SE2(), field_image, Vec3b(255, 0, 0), 1000,
                            20.0);  // target是蓝的
